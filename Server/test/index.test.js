...
describe('Test de RUTAS', ()=>{
    describe('GET /rickandmorty/character/:id', ()=>{
        ...
        it('Responde un objeto con las propiedades: "id", "name", "species", "gender", "status", "origin" e "image"', async ()=>{
            let { body } = await agent.get('/rickandmorty/character/2');
    ...
    describe('GET /rickandmorty/login', ()=>{
        it('Con el login correcto debe retornar access:true', async ()=>{
            let { body } = await agent.get('/rickandmorty/?email=nahuel@gmail.com&password=12345');
            expect(body.access).toBeTruthy();
        });
        it('Con el login incorrecto debe retornar access:false', async ()=>{
            let { body } = await agent.get('/rickandmorty/?email=evachiri@gmail.com&password=123456789');
